...
$editRow->newColumn()->size('col-2')->dbTable(TEXT_FIELD,'data_3')->type('select')->label('선택(Selector)')->addCol(___envArr('X009','env_prj.txt'));
$editRow->newColumn()->size('col-6')->dbTable(TEXT_FIELD,'data_4')->type('radio')->label('선택(Radio)')->addCol(___envArr('X009','env_prj.txt'));
$editRow->addRow()->hr();   //아래쪽에 구분선 출력

// Row 2
$editRow->newColumn()->size('col-6')->dbTable(TEXT_FIELD,'data_5')->type('checkbox')->label('선택(Checkbox)')->addCol(___envArr('X006','env_prj.txt'));
...
$editRow->newColumn()->size('col-4')->dbTable(TEXT_FIELD,'data_10')->type('steps')->label('진행상태(Step)')->addCol(___envArr('X011','env_prj.txt'));
...
// Row 5
$editRow->newColumn()->size('col-6')->dbTable(TEXT_FIELD,'data_15')->type('select')->label('일정구분')->addCol(___calendarGroupArr());
...
$editRow->newColumn()->size('col-4')->dbTable(DB_NUMBER_FIELD,'number_21')->type('inputSearch')->label('담당자')->default(0)->addCol($searchUser_forEdit,'담당자 검색','');
