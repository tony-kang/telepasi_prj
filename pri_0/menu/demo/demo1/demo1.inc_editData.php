...
// Row 4
$editRow->newColumn()->size('col-2')->dbTable(TEXT_FIELD,'data_12')->type(ET_INPUT)->label('회사명')->addCol();
$editRow->newColumn()->size('col-2')->dbTable(TEXT_FIELD,'data_13')->type(ET_EMPTY)->addCol();
$editRow->newColumn()->size('col-8')->dbTable(TEXT_FIELD,'data_14')->type(ET_INPUT)->label('회사주소')->placeHolder('주소를 입력하세요.')->addCol();
$editRow->addRow()->hr();

// Row 5
$editRow->newColumn()->size('col-6')->dbTable(TEXT_FIELD,'data_15')->type(ET_SELECT)->label('일정구분')->addCol(___calendarGroupArr());
$editRow->newColumn()->size('col-3')->dbTable(DATE_FIELD,'data_16')->type(ET_INPUT)->label('시작일')->alignCenter()->addCol();
$editRow->newColumn()->size('col-3')->dbTable(DATE_FIELD,'data_17')->type(ET_INPUT)->label('종료일')->alignCenter()->addCol();
...
$editRow->newColumn()->size('col-3')->dbTable(TIME_FIELD,'data_18')->type(ET_INPUT)->placeHolder('시작시간')->alignCenter()->addCol();
$editRow->newColumn()->size('col-3')->dbTable(TIME_FIELD,'data_19')->type(ET_INPUT)->placeHolder('종료시간')->alignCenter()->addCol();
$editRow->addRow()->hr();

// Row 7
include_once MY_SRC_PATH.'/search/searchUser_forEdit.php';
$editRow->newColumn()->size('col-4')->dbTable(DB_NUMBER_FIELD,'number_21')->type(ET_INPUT_SEARCH)->label('담당자')->default(0)->addCol($searchUser_forEdit,'담당자 검색','');
$editRow->newColumn()->size('col-6')->dbTable(URL_FIELD,'data_20')->type(ET_INPUT)->label('홈페이지(URL)')->addCol();
$editRow->addRow()->hr();

// Row 8
$editRow->newColumn()->size('col-6')->dbTable(EDITOR_FIELD,'text_31')->type(ET_HTML_EDITOR)->label('HTML 편집')->addCol();
$editRow->addRow();
