...
// Row 1
$editRow->newColumn()->size('col-2')->dbField(TEXT_FIELD,'data_1')->type(ET_INPUT)->label('테스트항목')->validator('테스트항목을 2자이상 입력해 주세요.','\'__V__\'.length > 1')->required(true)->note('일반입력(필수)')->addCol();
...
$editRow->newColumn()->size('col-3')->dbField(IMG_FIELD,'upload1')->type(ET_IMG_UPLOAD)->label('사진(1)')->note('사진파일 업로드')->addCol($userPhotoUpload,120,160);
$editRow->newColumn()->size('col-3')->dbField(IMG_FIELD,'upload2')->type(ET_IMG_UPLOAD)->label('사진(2)')->note('사진파일업로드')->addCol($userPhotoUpload_2,85,120);
$editRow->newColumn()->size('col-3')->dbField(IMG_FIELD,'upload3')->type(ET_DOC_UPLOAD)->label('이력서')->note('pdf 사용을 권장합니다.')->addCol($userProfileUpload,60,60);
$editRow->addRow()->hr();
