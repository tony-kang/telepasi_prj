...
function ___hoDrawing($ho,$w=60) {
    ...
    if (___isAdmin()) {
        //관리자는 호 레이아웃 셋팅메뉴 보임
        $hoEditUrl = '?cfg=dashboard&mN=complex&mS=complexHo&complex='.$ho['complex'].'&dong='.$encDong.'&edit=exist&ho='.$encHo;
        $aMenu->add(true,___amHref($hoEditUrl,sprintf('%s동 %s호 정보 수정',$ho['dong'],$ho['ho'])));

        $hoEditUrl = '?cfg=dashboard&mN=complex&mS=complexHoDetail&complex='.$ho['complex'].'&ho='.$encHo;
