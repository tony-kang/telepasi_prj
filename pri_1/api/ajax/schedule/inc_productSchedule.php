...
$_encProduct = $sO['schPara'] ?? '';
$productNo = $_encProduct ? ___getDecodeValue($_encProduct) : 0;
...
if ($productNo) {
    $sql->addWhere('and T.productNo='.$productNo);
}
...
foreach($_listArr['pageData'] as $data) {
    ...
    if ($processName) $data['title'] .= ' ● '.$processName;
