...
$sql->addTable('NEW_CONTT_MST_I', 'A');
$sql->addTable('CNTR_MST_HIS_I', 'B');
$sql->addTable('DONG_HO_MST_I', 'T');

// 테이블의 필드 가져오기
// 여러테이블을 중복하다보면 불필요한 필드를 제거해야 될 때가 많다. 일일이 타이핑 하기엔 필드가 너무 많아서...
//___debug(db_getTableField(S_DB,'NEW_CONTT_MST_I','NA'));
//___debug(db_getTableField(S_DB,'CNTR_MST_HIS_I','B'));
//___debug(db_getTableField(S_DB,'DONG_HO_MST_I','T'));

$sql->addField('A.COMPX_CD ,A.CONTT_NO ,A.BIZ_CD ,A.DONG_NO ,A.HO_NO ,A.CONTT_YMD ,A.CONTT_PATH ,A.SUP_TYPE ,A.CONTT_CHNG_RSN ,A.LEAS_STR_YMD ,A.LEAS_END_YMD ,A.LEAS_DPST_AMT ,A.PREMM_AMT ,A.RENT_AMT ,A.LNS_YN ,A.LNS_AMT ,A.LES_SET_YN ,A.RMK ,A.MVN_CK_YN ,A.REG_PSN ,A.REG_DT ,A.UPDT_PSN ,A.UPDT_DT ,A.WIN_TP ,A.MOVE_IN_START_YMD ,A.MOVE_IN_END_YMD ');
...
$sql->addWhere('and A.BIZ_CD = "RC"');

$sql->addWhere('and B.REPR_CNTR_YN = "Y"');
$sql->addWhere('and A.BIZ_CD = "NC"');
